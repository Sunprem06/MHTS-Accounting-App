...
import type { DocumentLayout, PurchaseInvoiceTemplateData } from './types';

function renderLineRows(data: PurchaseInvoiceTemplateData): string {
  return data.lines
    .map((line, index) => {
      const taxCell =
...
function renderTotalsRows(data: PurchaseInvoiceTemplateData): string {
  const rows: string[] = [];
  ...
  if (data.tdsAmount > 0) {
    rows.push(row(`TDS deducted${data.tdsSection ? ` (${escapeHtml(data.tdsSection)})` : ''}`, -data.tdsAmount));
    rows.push(
      `<tr><td colspan="6" style="text-align:right;padding:6px 8px;font-weight:bold;">Net payable</td><td style="text-align:right;padding:6px 8px;font-weight:bold;">${formatRupees(data.totalAmount - data.tdsAmount)}</td></tr>`,
    );
  }
  return rows.join('\n');
}

function renderClassic(data: PurchaseInvoiceTemplateData, accent: string): string {
  return `
    ${renderLetterheadBlock(data.letterhead, accent)}
    <hr style="border:none;border-top:2px solid ${accent};margin:12px 0;" />
    <div style="display:flex;justify-content:space-between;">
      <div>
        <h2 style="margin:0 0 4px 0;font-size:16px;">PURCHASE INVOICE${data.cancelled ? ' (CANCELLED)' : ''}</h2>
        <div><strong>Supplier:</strong> ${escapeHtml(data.partyName)}${data.isMsmeVendor ? ' (MSME)' : ''}</div>
      ...
      <div style="text-align:right;font-size:12px;">
        <div><strong>Bill No:</strong> ${data.voucherNumber}</div>
...
function renderModern(data: PurchaseInvoiceTemplateData, accent: string): string {
  return `
    ...
    <div style="display:flex;justify-content:space-between;margin-top:16px;">
      ...
      <div style="text-align:right;">
        <div style="font-size:18px;font-weight:bold;color:${accent};">PURCHASE INVOICE${data.cancelled ? ' — CANCELLED' : ''}</div>
        <div style="font-size:12px;">#${data.voucherNumber} &nbsp;•&nbsp; ${escapeHtml(data.invoiceDate)} &nbsp;•&nbsp; Due ${escapeHtml(data.dueDate)}</div>
      </div>
    </div>
    <div style="margin-top:16px;padding:12px;background:#f8f9fb;border-radius:6px;font-size:12px;">
      <strong>Supplier:</strong> ${escapeHtml(data.partyName)}${data.isMsmeVendor ? ' (MSME)' : ''}
...
export function renderPurchaseInvoiceHtml(data: PurchaseInvoiceTemplateData, layout: DocumentLayout): string {
  ...
  return wrapHtmlDocument(`Purchase Invoice ${data.voucherNumber}`, body);
}
