...
- [ ] Phase 4: **Input Tax Credit (ITC) eligibility/reversal is not built** — CGST/SGST/IGST/Cess Input ledgers correctly capture and classify recoverable tax paid on purchases, but there's no logic yet for Section 17(5) blocked-credit categories, matching against a supplier's GSTR-2B, or actually setting off input credit against output liability at return time. The GST Summary report shows both sides side-by-side but doesn't net them.
- [ ] Phase 4: **No GSTR-1/3B/9/9C filing-format prep** — the GST Summary report is a plain ledger reconciliation (output vs. input tax by CGST/SGST/IGST/Cess for a period), not the actual return-ready JSON/offline-tool format GSTN expects. Real filing prep (B2B/B2C invoice-level detail, HSN-wise summary, place-of-supply-wise breakup) is a follow-up pass.
- [ ] Phase 4: **No composition scheme or reverse-charge handling** — `business_party` has no GST registration-type field (regular/composition/unregistered), and reverse charge (where the recipient, not the supplier, is liable for GST) isn't modeled at all. Every party is implicitly treated as a normal registered dealer under the forward-charge mechanism.
...
- [ ] Phase 4: **No admin UI for GST-registration-type/party-level GST configuration beyond the existing gstin/state_code fields** carried over from Phase 2 — sufficient for this increment's place-of-supply logic, but GSTR-1's B2B-vs-B2C split (Phase 4's next increment) will need to know whether a customer is itself GST-registered.
- [ ] Phase 4: **Existing (pre-Phase-4) companies don't get GST permissions/ledgers retroactively** — `grantGstPermissions`/`seedGstLedgers` only run at NEW company creation, same as every prior module's permissions/ledgers (Phase 3's inventory ledgers had the identical characteristic). Not a real gap yet since no paying customers exist on an older schema version; would need a one-time backfill migration before it would matter.
