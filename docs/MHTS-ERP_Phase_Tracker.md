...
- [ ] Phase 11: **`sandbox: false` on the Electron `BrowserWindow` remains undocumented as a deliberate choice** — flagged in the security review (Section 3) but not investigated or changed this session; needs a decision on whether it was set for a specific reason (native module/preload requirement) or can safely become `sandbox: true`.
...
Any blockers (also add to Section 3): none new. The three real gaps the
  adversarial CA review found (RCM ITC set-off, ESI contribution-period
  continuity, Section 87A marginal relief) are flagged in the finalized pack
  but NOT fixed in code this session — CLAUDE.md requires explicit user
  confirmation before touching financial/GST/payroll logic, and this was a
  document-production session. They should be scoped and confirmed as a
  real follow-up increment.
Next concrete step: the user should (1) do a visual open-in-Word pass on all
  six generated documents before sending any of them onward, given this
  session had no rendering tool to verify formatting itself; (2) decide
  whether/when to scope a follow-up increment fixing the RCM ITC set-off
  question, the ESI contribution-period gap, and the Section 87A marginal-
  relief gap in actual code; (3) whenever ready, everything from both
  Increment 1 and Increment 2 is still on one unmerged branch
