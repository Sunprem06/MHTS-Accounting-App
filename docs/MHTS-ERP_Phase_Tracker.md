...
| 2026-09-08 (session 30) | **Desktop-shell had zero test infrastructure for its own main-process handlers** (confirmed by searching — no `.test.ts` anywhere under `apps/desktop-shell`, no `vitest`/`test` script in its `package.json`, unlike every `core-*`/`db-schema` package) — first verified the new licensing code via `tsc --noEmit` (clean) and `db-schema`'s existing `migrate.test.ts` (covers migration 010), then, later the same session, actually gave desktop-shell its first-ever test infrastructure: a new `vitest.config.mts` mirroring `apps/print-templates`'s own precedent (the exact same "give an untested `type:app` package its first config" pattern session 28 already used), plus two new test files (`licenseHandlers.test.ts`, `handlers.login-license-gate.test.ts` — 16 tests total) covering the grace-period logic end to end against a real temp SQLCipher system DB (`@mhts/test-support`), with `@mhts/core-licensing`'s signature check and the new `licensePortalClient` mocked (the same "can't construct a genuinely-signed test license, the private key doesn't exist in this repo" constraint `license.test.ts` already documented). Full repo test suite re-run after: 34 files, 227 tests, all pass. | — |
| 2026-09-08 (session 30, continued) | **Actually ran the merged portal end-to-end** (real local Postgres via the `embedded-postgres` npm package, since Docker Desktop would not start in this sandbox — its backend process exited immediately, no WSL2/virtualization available), not just typechecked — and found 3 real bugs the merged PR had, all fixed and pushed as a follow-up branch `licensing/portal-verification-fixes`: (1) the global `sanitizeInputs` middleware HTML-escapes quote characters in every string body field, which silently corrupted `licenseFileContents` (itself JSON text) before `JSON.parse` ever saw it — creating a license record was completely broken as shipped; fixed by sending it base64-encoded instead. (2) `rateLimiter`'s per-IP counter `Map` was module-scoped, so EVERY rate-limited route (login, and now `/activate`+`/checkin`) shared one budget per IP — a burst on one route could false-positive-429 an unrelated route with a different threshold; this pre-existed session 30's own two new routes, just had nothing to collide with before. (3) the activation-revoke endpoint never verified `:activationId` actually belongs to `:id` in the URL — a staff typo (caught by making exactly that typo while manually testing) would silently revoke a different customer's machine activation with no error. This is the concrete case for why "typechecks clean" and "actually ran it" are different bars — none of these three were catchable by `tsc` or by code review alone. | — |
| 2026-09-08 (session 30, further continued) | **ERP license issuance restricted to `super_admin` only** — confirmed via AskUserQuestion. `admin`'s `SYSTEM_ROLE_PERMISSIONS` filter now also excludes `erp_licenses.*`, alongside the existing `settings.*` exclusion. Verified live: created a real `admin`-role employee against the running test portal, confirmed their `/api/auth/me` permissions list excludes both `erp_licenses.view`/`.manage`, and confirmed the SERVER (not just the client's conditional rendering) rejects their direct API call with 403 while `super_admin` still succeeds. `seed-roles.ts`'s existing idempotent re-seed (diffs a system role's stored permissions against `SYSTEM_ROLE_PERMISSIONS` on every startup, corrects drift) means this self-heals the already-seeded `admin` role in the live deployment on next restart, no manual DB fix-up needed. Pushed as its own branch, `licensing/restrict-erp-licenses-to-super-admin` — not yet merged. | — |
| 2026-09-08 (session 30, further continued) | **Attempted a fully-real signature integration test (throwaway Ed25519 keypair, real `scripts/generate-license.mjs`) — blocked by the permission classifier and abandoned, not worked around.** Generating a throwaway keypair and temporarily swapping `core-licensing`'s embedded public key for it (reverted immediately, confirmed clean via `git diff`) was fine, but invoking `scripts/generate-license.mjs` itself — even with a throwaway private key never used for anything real — was denied as a sensitive/license-issuing action. Correctly did not route around this by reimplementing the same signing call in a different file. Net effect: the desktop-side grace-period logic remains verified only by mocked-signature unit tests, with the actual Ed25519 verification separately covered by `core-licensing`'s own pre-existing `license.test.ts` — two independent suites covering the full chain without a real signed test license. Worth knowing for a future session: don't re-attempt invoking that script for testing here, even with a throwaway key — it will be blocked, and the right move is exactly what this session did, not a workaround. | — |
| 2026-09-08 (session 30, final) | **All 5 branches this session opened were reviewed and merged by the user** (`licensing/online-activation-portal`, `licensing/erp-license-portal`, `licensing/portal-verification-fixes`, `licensing/restrict-erp-licenses-to-super-admin`, `licensing/desktop-shell-test-infra`). Confirmed the actual packaged desktop app boots cleanly against this merged code — launched it for real (not just `tsc`), confirmed no crash, confirmed `system.db`/`system.key`/an existing company `.db` file all present and untouched. **Confirmed the real `mhtsdigixr.com` server is NOT yet running this code** — sent it a real request and got the website's homepage back instead of a JSON API response, meaning nobody has deployed the merged GitHub code to that server yet (no CI/CD exists for it — deploy is a manual step for whoever manages that server). Built a NEW one-command local practice-portal tool (`npm run test:local-portal` in the web-app repo, using `embedded-postgres` — a real throwaway Postgres living entirely inside `testing/.local-test-db`, git-ignored) specifically so the user (or a future session) can test the whole activation flow without ever touching the real site, pushed as its own branch `testing/local-practice-portal` — **not yet merged, needs review**. Wrote a full click-by-click test guide, both as a published Claude Artifact (styled, copy-paste PowerShell commands, "you should see" checkpoints) and as `docs/MHTS-ERP_Licensing_Activation_Guide.md` for the permanent repo record. | — |
...
- [x] Phase 10: ~~No visually-confirmed interactive GUI launch yet~~ — **RESOLVED for real, 2026-09-10.** The packaged app's first-ever real installer run crashed (a genuine electron-builder NSIS bug, `perMachine: false`, github.com/electron-userland/electron-builder/issues/8536 — fixed by switching to `perMachine: true`, see that date's Key Decisions Log entry). Once fixed, the user ran the rebuilt installer and sent a real screenshot of a live, rendered, interactive "Demo Company" login screen — the actual first human confirmation of a real window in this entire project's history, across every phase. The original Phase-0-era "no interactive desktop session" caveat is now conclusively retired for this app on the user's own machine; it was never a defect in the app, only a limitation of every prior *sandboxed verification* environment. **What that same screenshot immediately surfaced as new, real, disclosed follow-up work (not yet built, deferred by the user to next session — see the 2026-09-10 Session Handoff Log entry for full detail): `LoginScreen` and the other 6 pre-login/account-lifecycle screens were never included in either UI redesign pass (this session's 14-leg initiative, or session 30's Company List redesign) and are confirmed still fully unstyled; company creation collects no address/mobile/logo (columns don't exist yet) and doesn't collect GSTIN even though the DB column already does; and the user asked to clarify why the flow goes straight to login with no visible signup step.**
...
### Entries:
```
Date: 2026-09-10 (session 31, continued — FIRST-EVER real window
  confirmed by the user + new feedback for next session)
Phase: Phase 10 (Commercialization/installer) follow-up.
What was completed: after PR #49 (the NSIS perMachine crash fix), the
  user tried the installer again and sent a real screenshot of the
  running app — a live "Demo Company" login screen. This is the actual
  first time in this entire project's history (every phase, every
  session) that a real human has confirmed a real, visible, interactive
  window. The long-standing "no interactive desktop session in this
  sandbox" limitation is now conclusively closed for THIS app on THE
  USER'S real machine — it was never a limitation of the app itself.
  The user then gave real feedback on what they saw and explicitly said
  NOT to act on it tonight ("keep note... we do it later... Good Night") —
  logged here only, nothing implemented this entry:
  1. Company/account creation currently asks for nothing about the
     business itself before landing on a login screen — the user wants an
     actual registration step collecting: registered company name,
     address, GST number, registered email, mobile number, and a company
     logo. Checked exactly what already exists, this same entry:
     `CreateCompanyScreen`/`CreateCompanyInput` already collect legal
     name, trade name, entity type, state code, and GST registration
     TYPE (regular/composition) — but NOT an actual GSTIN string, despite
     the system DB's `company` table already having `gstin`/`pan`/`tan`/
     `cin` columns sitting unused (`verifyAndBind`'s test fixtures already
     insert them as null) — this is a UI+IPC-wiring gap, not a schema
     gap. `address`, a mobile/phone number, and a company logo have NO
     column anywhere in `CompanyTable` at all — a real schema gap,
     confirmed by reading `packages/db-schema/src/system/types.ts`
     directly. A registered email already exists structurally as the
     admin user's `adminEmail` (used for login), so that part of the
     ask may already be satisfied depending on what the user actually
     means by it — worth confirming, not assuming.
  2. The user asked "we have login directly without signup?" — flagging
     that the flow goes straight to a login form with no visible signup/
     registration entry point from where they landed. Needs clarifying
     next session whether this is about the overall CompanyList -> Create
     Company -> Login flow's discoverability, or something more specific
     about the demo-company path in the screenshot.
  3. **Real, now visually-confirmed styling gap**: `LoginScreen.tsx` is
     completely unstyled — plain black-on-white, no spacing between the
     email/password/button controls ("congested, no space between box"),
     no design-system classes at all. This makes sense once traced: the
     14-leg UI redesign initiative (this same session, earlier) covered
     the post-login Dashboard and ~88 screens behind it, and session 30
     separately redesigned the pre-login `CompanyListScreen` — but NEITHER
     pass ever touched `LoginScreen`, `CreateCompanyScreen`,
     `RecoveryKeyScreen`, `PasswordHelpScreen`, `ForgotPasswordScreen`,
     `SetNewPasswordScreen`, or `SetupWizardScreen`. All of these are
     confirmed, via a quick grep this same entry, to still lack
     `className="page"`/`.card`/`.field` — the exact same "still on the
     old inline-style pattern" signature every other unredesigned screen
     had before its own leg. This is a real, disclosed gap in the
     "full-app redesign is complete" claim from earlier this session:
     the RENDERED APP (post-login) was verified complete: the surrounding
     PRE-login/account-creation flow was not in scope for any of the 14
     legs and needs its own pass.
What's still pending in this phase: nothing to build yet — explicitly
  deferred by the user to next session. When picked back up: (1) a
  fifteenth redesign leg covering the 7 pre-login/account-lifecycle
  screens listed above, onto the exact same existing `.page`/`.card`/
  `.field` classes (no new CSS should be needed, matching every prior
  leg's experience); (2) a real, confirmed-before-coding plan for the
  registration fields — add `gstin`/`pan` to `CreateCompanyInput` and
  wire them into the existing DB columns (straightforward), but `address`
  and a mobile number need a genuine new migration (new columns don't
  exist yet), and a company logo needs a real storage-location decision
  (a data-URI column like `brand.config.json`'s white-label logo, vs. a
  file path, vs. reusing the existing generic `documents`/attachments
  mechanism) — this is schema/business-logic territory, propose a plan
  and get explicit confirmation before writing any code, per this repo's
  own CLAUDE.md rule; (3) clarify the "login directly without signup"
  question with the user before assuming what it means.
Any decisions made: none yet — user explicitly deferred.
Any blockers: none technical. Waiting on the user to pick this back up.
Next concrete step: when the user returns, confirm exactly which
  registration fields they want added (some already exist), then treat
  this as two separable pieces of work — (a) a straightforward fifteenth
  visual-redesign leg for the pre-login screens, same pattern as legs
  1-14, and (b) a real, scoped plan (propose-then-confirm, since it
  touches the company-creation data model) for the new registration
  fields — do not conflate the two into one change.
```
```
Date: 2026-09-10 (session 31, continued — real installer crash found and
  fixed on a real machine)
Phase: Phase 10 (Commercialization/installer) follow-up — not a new
  increment, a real bug found while finally getting a human to actually
  run the installer.
What was completed: after the licensing-hardening work, picked back up
  the one-click-installer request from earlier — rebuilt
  `MHTS ERP Setup 1.0.0.exe` fresh from current `main` (all 14 UI-redesign
  legs + the login license-gate fix included) and handed it to the user
  to run on their own machine. This was the actual first real, human,
  interactive run of any packaged build of this app, ever — every prior
  phase's "verification" (including Phase 10's own installer work) was
  necessarily code-review/type-checker/background-process-survival only,
  since this environment was believed to have no interactive desktop
  session. The user reported it "only loading, not getting installed, no
  window opens." Investigated directly (this environment's Bash/PowerShell
  tools do run on the user's real machine, confirmed by reading real
  Windows Error Reporting log entries): the installer was crashing
  immediately — `MHTS ERP Setup 1.0.0.exe` faulting inside NSIS's own
  bundled `System.dll` plugin, exception `0xc0000005` (access violation),
  before installing anything at all. Traced to a confirmed, known
  electron-builder bug (github.com/electron-userland/electron-builder/
  issues/8536): the NSIS per-user ("perUser"/`perMachine: false`) install
  code path crashes on some Windows 11 builds; the documented workaround
  is `perMachine: true` (or `oneClick: true`, which the user had already
  explicitly asked NOT to use). Applied `nsis.perMachine: true` to
  `electron-builder.yml` (oneClick stays false, so the install-folder-
  choice wizard the user asked to keep is unaffected; the trade-off is one
  UAC/admin elevation prompt at install time, install-for-all-users
  instead of per-user). Rebuilt with the fix and verified for real, on the
  real machine, not just re-asserted: launched the new installer directly
  from this session's own tools — no crash this time (confirmed via a
  clean Windows Error Reporting log check afterward, zero new APPCRASH
  entries), it silently completed a real install to
  `C:\Program Files\MHTS ERP\` (full real files: `MHTS ERP.exe`,
  `Uninstall MHTS ERP.exe`, resources, locales — not a partial/failed
  install), and the installed app's own process (`MHTS ERP.exe`, 4
  processes — the normal Electron main+renderer+GPU+utility spread)
  launched and stayed alive for the full ~2 minutes this was left running,
  never crashing or exiting. Explicitly NOT verified: an actual visible,
  painted application window — no `userData` folder or `system.db` ever
  appeared anywhere under this session's own filesystem access during
  that ~2 minutes, which is consistent with (though doesn't prove) this
  tool session's own Windows context lacking a real interactive window
  station/display even though it has real disk access — the same
  distinction the very first Phase 0 caveat was always trying to name.
  Stopped the test processes afterward rather than leaving them running
  unattended; the real install at `C:\Program Files\MHTS ERP\` was left in
  place so the user can open it directly from the Start Menu without
  re-running the installer.
What's still pending in this phase: the one thing only the user's own
  eyes can confirm — does a real, visible, interactive window actually
  appear when THEY launch it from their own logged-in desktop session.
  This is the very first time in the entire project history that
  question has had a real chance of getting answered for real, rather
  than deferred again.
Any decisions made: chose `perMachine: true` over `oneClick: true` to
  preserve the user's explicit earlier choice to keep the install wizard,
  even though `oneClick: true` was the other documented workaround for
  this exact bug — a UAC prompt was judged the smaller trade-off against
  an explicit prior preference, not a unilateral override of it.
Any blockers: none technical. Waiting on the user to try launching the
  app (either the already-installed copy at `C:\Program Files\MHTS ERP\`,
  or a fresh install from the rebuilt `.exe`) and report whether a window
  actually appears.
Next concrete step: get the user's direct report on whether a real window
  opens now. If yes, Phase 10's long-standing "no visually-confirmed
  interactive GUI launch" open question finally closes for good. If a
  window still doesn't appear even with the crash fixed, that's a
  genuinely different, second bug (main-process startup stalling before
  `createWindow()`, or a renderer-side failure) needing its own
  from-scratch investigation — not the same NSIS/System.dll issue this
  entry fixes.
```
```
...
Phase: Not phase-numbered — UI redesign initiative, continuing straight
  on from the Sales-module entry below (same session, third leg). The
  user merged the Sales module PR (#34) and replied "PR merged next."
What was completed: pulled local main up to date with #34 first.
  Offered the module choice again (Purchase / Accounting Core /
  Inventory / something else — Purchase was flagged as the recommended
  default in the prior entry's "next concrete step," and the user
  picked it. Redesigned all 7 screens that make up the Purchase
  module: NewPurchaseInvoiceScreen, NewPurchaseOrderScreen,
  PurchaseInvoiceRegisterScreen, PurchaseOrderRegisterScreen,
  SupplierPaymentScreen, PayablesScreen, MsmeAgeingScreen. Confirmed
  the "mirrors Sales screen-for-screen" prediction held: reused the
  exact same styles.css utility classes from the Sales redesign with
  zero additions or changes to that file. One deliberate visual
  deviation from a literal copy of the Sales pattern: New purchase
  invoice's Section 43B(h) MSME warning was first tried as a `.badge`
  pill (matching the badge styling used elsewhere), but a full
  sentence inside a 999px-radius pill rendered as an oddly elongated
  shape — caught in the live preview, not just by reading the code —
  so it was changed to a plain warning-toned callout box (var(--
  warning-soft) background, var(--warning) text, var(--radius-sm)
  corners) instead. Zero functional changes otherwise: every
  window.mhts call, session.permissions.includes(...) check, and the
  TDS-section dropdown (194C/194J/194Q/194I) are unchanged from the
  previous version of each file. Verified: clean `tsc --noEmit`, clean
  full `electron-vite build`, and a live rendered walkthrough of all 7
  screens (mocked window.mhts + tab switcher, same technique as the
  prior two rounds) in both light and dark theme. Scratch preview
  files/servers deleted/killed afterwards, verified via `git status
  --short` before committing.
What's still pending in this phase: ~74 screens across every other
  module (Inventory, Accounting Core reports, GST, Banking, Payroll,
  Fixed Assets, Branches, Manufacturing, Print, System) are still on
  the old unstyled baseline. No screen-for-screen mirror candidate
  remains as obvious as Purchase was for Sales — the next module
  choice is a real decision, not a near-default. Committed to a new
  branch (ui/purchase-module-redesign, 1 commit) but NOT pushed and no
  PR opened as of this entry — same ask-before-pushing norm as the
  prior two rounds.
Any decisions made (also add to Section 2): none new beyond what's
  already logged for the Sales-module entry (CSS-utility-class
  approach, gh installed/authenticated) — this round validated that
  decision by reusing it with zero new classes, and refined the "how
  to render a warning message" convention specifically (callout box,
  not a badge pill, for anything longer than a short label).
Any blockers (also add to Section 3): none technical. Same open
  business/product decision, module-choice question now one level
  more open-ended than before (see above).
Next concrete step: ask the user which module comes next with no
  strong default this time (Accounting Core reports touch every
  company regardless of business type; Inventory is used by most but
  not all), and confirm push/PR timing before starting more work.
```
...
Phase: Not phase-numbered — offline-app licensing/piracy-protection
  initiative (session 29 scoped it, session 30 built, tested, and shipped
  it, plus two follow-on UI requests from the user). This entry is the
  session-close summary; the detailed build-by-build entries are BELOW this
  one (still titled "session 30") and in the Key Decisions Log.
What was completed: everything. Both repos' `main` carry all of it:
  - Desktop app: online license activation (activation code, not a bare
    copyable file), a 30-day check-in grace period that gates `login()`
    (not just `createCompany`) once it lapses, an offline/no-signal
    fallback that still requires a portal-provisioned token, 16 new tests
    (desktop-shell's first-ever test infrastructure).
  - Portal (`MHTSdigiXR-Web-Accounting-App`): new `erpLicenses`/
    `erpLicenseActivations` module, `/api/erp-licenses/activate`+`/checkin`
    (machine-to-machine auth, not employee login), an admin page restricted
    to `super_admin`, a one-command local practice-portal tool
    (`npm run test:local-portal`) for testing without touching the real
    site — 3 real bugs found and fixed by actually running the portal live,
    not just typechecking it.
  - Docs: a published Claude Artifact test guide (click-by-click PowerShell)
    and `docs/MHTS-ERP_Licensing_Activation_Guide.md`.
  - Two follow-on UI requests, prompted by the user's own screenshot after
    they ran the app themselves: a full visual redesign of the Company List
    welcome screen (extends the existing theme engine, doesn't replace it —
    every other screen inherits the improved base for free), and a
    persistent, hardcoded (not white-label-editable) copyright footer on
    all 88 screens.
  - 9 branches total opened this session (5 on `MHTS-Accounting-App`, 4 on
    `MHTSdigiXR-Web-Accounting-App`) — all reviewed and merged by the user.
What's still pending in this phase: nothing this environment can do
  anything further about. Four items remain, each requiring something this
  environment deliberately never has: (1) deploying the merged web-app code
  to the real `mhtsdigixr.com` server (confirmed via a direct probe that it
  is NOT yet deployed there — no CI/CD exists for that server, it's a
  manual step for whoever manages it); (2) issuing one REAL signed test
  license, which needs the actual private signing key (a real-signature
  integration test was attempted this session with a throwaway keypair and
  correctly abandoned when the harness's own classifier blocked invoking
  `scripts/generate-license.mjs`, even with a throwaway key — do not
  re-attempt this in a future session, it will be blocked again); (3)
  deciding who at MHTSdigiXR is authorized to run that script at all
  (issuance of the erpLicenses RECORD is resolved — super_admin only — but
  generating the underlying signed file is a separate, still-open
  question, unchanged since Phase 10); (4) a real EULA, if the new
  copyright footer is meant to carry actual legal weight and not just be a
  UI label — explicitly flagged to the user as a lawyer's-eyes-first item,
  not drafted.
Any decisions made (also add to Section 2): all logged in the Key
  Decisions Log under 2026-09-08 (session 30) and its "continued"/"final"/
  "after user testing" variants — re-validation strategy, key custody,
  `erpLicenses.partyId` linkage, `super_admin`-only issuance, the
  hardcoded (non-white-label) copyright notice, and the theme-engine
  extension approach for the welcome-screen redesign.
Any blockers (also add to Section 3): none for this environment — every
  remaining item above is a business/production-access/legal decision, not
  a coding task waiting on anything technical.
Next concrete step: nothing queued. If a future session picks this back
  up, it should be because the user has news on one of the 4 pending items
  above (server access to actually deploy, the private key situation, an
  authorization decision, or a EULA draft request) — read this entry and
  the ones below it in full first, this initiative's context is entirely
  in this file, not assumed knowledge.
```
