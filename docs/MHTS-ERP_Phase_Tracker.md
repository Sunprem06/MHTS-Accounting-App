...
- [ ] New initiative, not phase-numbered: **Offline-app licensing/piracy protection — core feature MERGED and live (session 30, 2026-09-08); three follow-up branches pushed, NOT YET MERGED. See the full session 30 entries at the TOP of Section 4 before doing anything here.** Merged: this repo's `licensing/online-activation-portal` and `MHTSdigiXR-Web-Accounting-App`'s `licensing/erp-license-portal`. The portal was then actually stood up against a real local Postgres (via `embedded-postgres`, since Docker Desktop wouldn't start in this sandbox) and exercised end-to-end via real HTTP requests, which found and fixed 3 real bugs (sanitizeInputs corrupting the license file JSON, a module-scoped rate-limiter Map shared across unrelated routes, an unverified cross-license activation-revoke path) — pushed as `licensing/portal-verification-fixes`. The issuance-authorization question was resolved (super_admin only, confirmed via AskUserQuestion, verified live) — pushed as `licensing/restrict-erp-licenses-to-super-admin`. Desktop-shell also gained its first-ever test infrastructure (16 tests) — pushed alongside the Phase Tracker updates as `licensing/desktop-shell-test-infra`. **All three of those branches need review/merge before this initiative is fully done.** `MHTSdigiXR-Web-Accounting-App` is a fresh local clone at `D:\MHTS-SaaS-Projects\2026\MHTSdigiXR-Web-Accounting-App`, not otherwise part of this codebase/working directory — don't assume it's there unless you (re-)verify. Only genuinely remaining item after those merges: a live click-through of the new Company List activation UI on the user's own machine — the desktop side was never run against the real portal end-to-end in this sandbox (a real signature integration test was attempted and correctly abandoned when the harness's classifier blocked invoking `scripts/generate-license.mjs`, even with a throwaway keypair — see the dedicated Key Decisions Log row on this; don't re-attempt it in a future session).
