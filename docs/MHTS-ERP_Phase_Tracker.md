...
- [ ] Phase 9: **Printing/PDF only covers Sales Invoice and Payslip this increment** — Purchase Invoice, Sales/Purchase Orders, Payment/Receipt/Contra/Journal vouchers, and Expense Claims have no print/PDF path yet. This is the confirmed scope of Increment 2, not a gap within Increment 1.
- [ ] Phase 9: **No Print Centre register screen yet** — there is no central place to browse every past printed/printable document and reprint it; printing today is only reachable per-row from the Sales Invoice Register and the Payroll Run screen. Also confirmed Increment 2 scope.
- [ ] Phase 9: **No drag-and-drop template designer** — the two layouts (CLASSIC/MODERN) per document type are fixed HTML/CSS; a company can rebrand (logo/address/bank/footer/accent/layout choice) but cannot reposition or add arbitrary fields. Confirmed Increment 3 scope (the user explicitly wants this eventually, not skipped).
