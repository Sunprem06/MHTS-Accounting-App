...
- [ ] New initiative, not phase-numbered: **Offline-app licensing/piracy protection — both original PRs MERGED (session 30, 2026-09-08): this repo's `licensing/online-activation-portal` and `MHTSdigiXR-Web-Accounting-App`'s `licensing/erp-license-portal`. See the full session 30 entries (two of them — the initial build, then "continued") at the TOP of Section 4 before doing anything here.** The portal was then actually stood up against a real local Postgres (via `embedded-postgres`, since Docker Desktop wouldn't start in this sandbox) and exercised end-to-end via real HTTP requests — this found 3 real bugs the merged PR had (sanitizeInputs corrupting the license file JSON, a module-scoped rate-limiter Map shared across unrelated routes, and an unverified cross-license activation-revoke path), all fixed and pushed as a third branch, `licensing/portal-verification-fixes`, **NOT YET MERGED — needs review/merge before this is actually done.** `MHTSdigiXR-Web-Accounting-App` is a fresh local clone at `D:\MHTS-SaaS-Projects\2026\MHTSdigiXR-Web-Accounting-App`, not otherwise part of this codebase/working directory — don't assume it's there unless you (re-)verify. Desktop-shell also gained its first-ever test infrastructure this session (16 new tests covering the grace-period logic against a real temp system DB). Remaining before this is truly done: (1) review/merge `licensing/portal-verification-fixes`; (2) a live click-through of the new Company List activation UI on the user's own machine — the desktop side itself was never run against the real portal end-to-end (only the portal's own routes were exercised directly via curl, and the desktop logic was verified via mocked-signature unit tests, not a real signed license — the private key doesn't exist in this repo); (3) decide who at MHTSdigiXR is authorized to create `erpLicenses` records / hand out activation codes, mirroring the still-open question about who's authorized to run `scripts/generate-license.mjs` at all.
