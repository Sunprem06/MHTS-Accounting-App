...
- [ ] New initiative, not phase-numbered: **Offline-app licensing/piracy protection — ALL CODE MERGED on both repos (session 30, 2026-09-08). Only remaining items are deploy-to-production (not this environment's to do) and one more small branch pending review. See the full session 30 entries at the TOP of Section 4 before doing anything here.** Merged on `MHTS-Accounting-App`: `licensing/online-activation-portal`, `licensing/desktop-shell-test-infra`. Merged on `MHTSdigiXR-Web-Accounting-App`: `licensing/erp-license-portal`, `licensing/portal-verification-fixes` (3 real bugs found by actually running the portal), `licensing/restrict-erp-licenses-to-super-admin`. **Confirmed the actual packaged desktop app boots cleanly against the merged code** (launched for real, not just typechecked — no crash, existing `system.db`/company data untouched) and **confirmed the real `mhtsdigixr.com` server has NOT been deployed with any of this yet** (probed it directly — it still returns the old website, not the new API routes; no CI/CD exists for that server, deploy is a manual step for whoever manages it). One more branch pushed but not yet merged: `testing/local-practice-portal` (a new one-command local practice-portal tool, `npm run test:local-portal`, so anyone can test the whole flow without touching the real site). Full test guide written: a published Claude Artifact (click-by-click, copy-paste PowerShell) and `docs/MHTS-ERP_Licensing_Activation_Guide.md` for the permanent record. Genuinely remaining, and outside what this environment can do: (1) merge `testing/local-practice-portal`; (2) deploy the merged web-app code to the real server; (3) issue one REAL signed test license (needs the actual private key — a real signature integration test was attempted here with a throwaway keypair and correctly abandoned when the harness's classifier blocked invoking `scripts/generate-license.mjs` even with a throwaway key — don't re-attempt that in a future session, see the dedicated Key Decisions Log row); (4) decide who's authorized to run `scripts/generate-license.mjs` at all — issuance of the erpLicenses RECORD itself is now resolved (super_admin only), but generating the underlying signed file is a separate, still-open question, same as it's been since Phase 10.
